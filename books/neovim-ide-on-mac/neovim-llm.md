...
- **チャット** (`<leader>ac`): VS Code のチャットパネルに相当します。バッファとして開くので、`gd` 等の編集操作がそのまま効きます
- **インライン** (`<leader>ai`): Visual で範囲選択して「これを RxJS で書き直して」等を指示すると、その場に反映されます
- **アクション** (`<leader>aa`): 「コードを説明」「テストを生成」等の定型タスクをメニューから選べます
...
これで Neovim 関連の章は一通り終わりです。Neovim の基礎素振りから、IDE 化・tmux 統合・リモート開発・ローカル LLM 連携まで、ぐるりと一周してきました。お疲れさまでした。
